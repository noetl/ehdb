...
use ehdb_reference::{
    ...
    exercise_durable_recovery, exercise_primary_serve, exercise_segment_gc, exercise_shared_tier,
    exercise_shared_tier_gc, ingest_local_reference_retrieval_document_json,
    ...
    summarize_local_reference_json, AckEventConsumerRequest, AffinityRoutedEventLog,
    AppendDomainRecordRequest, AuthoritativeExecutionState, BindSystemChannelRequest,
    ConsumeEventRecordsRequest, EventLogAckRequest, EventLogAppendRequest, EventLogDriver,
    EventLogPrimaryEvent, EventLogReadExecutionRequest, EventLogScanRequest, EventLogTailRequest,
    IngestChunkInput, IngestRetrievalDocumentRequest, KvPrimaryInput, LocalReferenceEventLogDriver,
    LocalReferenceKvStateDriver, LocalReferenceObjectBlobDriver, LocalReferenceProjectionEngine,
    LocalReferenceVectorDriver, ObjectPrimaryInput, ProjectionApplyRequest, ProjectionDriver,
    ProjectionEventInput, ProjectionPrimaryInput, PublishSystemModuleRequest,
    ReadDomainRecordsRequest, ResolveSystemModuleRequest, RetrievalOutcome, RetrieveContextRequest,
    Routed, SegmentGcPolicy, ServedBy, ShardOwnership, VectorPrimaryInput,
...
fn run(args: Vec<String>) -> Result<(String, i32), String> {
    match args.split_first() {
        ...
        Some((command, rest)) if command == "durable-eventlog-shared" => {
            run_durable_eventlog_shared(rest)
        }
        Some((command, rest)) if command == "durable-eventlog-gc" => run_durable_eventlog_gc(rest),
        Some((command, rest)) if command == "durable-eventlog-shared-gc" => {
            run_durable_eventlog_shared_gc(rest)
        }
...
/// Shared / object-store segment-tier drive (completion program, durable
/// event-log backend slice 3): spin up a `--shard-count`-replica pool with
/// SEPARATE local disks over one shared store under `--root`, and prove
/// owner-publishes-to-shared / non-owner-cold-loads-from-shared /
/// crash-recovery-from-shared (a new owner with an empty local disk hydrates the
/// shard) / shared-store-miss-reads-empty / parity.  Exit 0 only when
/// [`SharedTierReport::holds`] holds.
fn run_durable_eventlog_shared(args: &[String]) -> Result<(String, i32), String> {
    let mut flags = parse_flags(args)?;
    let root = take_required(&mut flags, "root")?;
    let shard_count = match flags.remove("shard-count") {
        Some(raw) => raw
    ...
    match exercise_shared_tier(PathBuf::from(root), shard_count) {
        Ok(report) => {
            let holds = report.holds();
            let output = serde_json::to_string(&serde_json::json!({
                "suite": "ehdb-durable-eventlog-shared",
                "shard_count": report.shard_count,
                "shared_tier_holds": holds,
                "report": report,
            }))
            .map_err(|err| err.to_string())?;
            Ok((output, if holds { 0 } else { 1 }))
...
/// Segment-GC drive (completion program, durable event-log backend segment-GC
/// slice; the runbook §C residual-risk R1 / D11 gap): append `--events` under a
/// small `--segment-max-bytes`, ack a durable consumer to ~3/4 of the log,
/// reclaim consumed sealed segments (keeping `--min-retained-segments`), and
/// prove growth was bounded while the retained log stayed gapless-from-base +
/// zero-loss and a restart recovers. Exit 0 only when
/// [`SegmentGcDriveReport::holds`] holds.
fn run_durable_eventlog_gc(args: &[String]) -> Result<(String, i32), String> {
    let mut flags = parse_flags(args)?;
    let root = take_required(&mut flags, "root")?;
    let events = match flags.remove("events") {
        Some(raw) => raw
            .parse::<usize>()
            .map_err(|_| format!("invalid --events value: {raw}"))?,
        None => 40,
    };
    let segment_max_bytes = match flags.remove("segment-max-bytes") {
        Some(raw) => raw
            .parse::<u64>()
            .map_err(|_| format!("invalid --segment-max-bytes value: {raw}"))?,
        None => 220,
    };
    let min_retained = match flags.remove("min-retained-segments") {
        Some(raw) => raw
            .parse::<usize>()
            .map_err(|_| format!("invalid --min-retained-segments value: {raw}"))?,
        None => 2,
    };
    let consumer = flags
        .remove("consumer")
        .unwrap_or_else(|| "gc-projector".to_string());
    ensure_no_unknown_flags(&flags)?;

    let policy = SegmentGcPolicy::enabled(min_retained);
    match exercise_segment_gc(
        PathBuf::from(root),
        events,
        segment_max_bytes,
        &policy,
        ...
        Ok(report) => {
            let holds = report.holds();
            let output = serde_json::to_string(&serde_json::json!({
                "suite": "ehdb-durable-eventlog-gc",
                "driver": report.driver_name,
                "gc_holds": holds,
                "report": report,
            }))
...
/// Shared-tier segment-GC drive (completion program, durable event-log backend
/// shared-tier segment-GC slice): a `--shard-count`-replica pool over one shared
/// store reclaims a shard's consumed sealed segments across BOTH local + shared,
/// and proves a non-owner cold-load AND a fresh new-owner hydrate both see
/// exactly the owner's retained set (no re-pull of reclaimed segments, no
/// un-reclaim on transfer, shared disk bounded). Exit 0 only when
/// [`SharedTierGcReport::holds`] holds.
fn run_durable_eventlog_shared_gc(args: &[String]) -> Result<(String, i32), String> {
    let mut flags = parse_flags(args)?;
    let root = take_required(&mut flags, "root")?;
    let shard_count = match flags.remove("shard-count") {
        Some(raw) => raw
    ...
    match exercise_shared_tier_gc(PathBuf::from(root), shard_count) {
        Ok(report) => {
            let holds = report.holds();
            let output = serde_json::to_string(&serde_json::json!({
                "suite": "ehdb-durable-eventlog-shared-gc",
                "shard_count": report.shard_count,
                "shared_gc_holds": holds,
                "report": report,
            }))
            .map_err(|err| err.to_string())?;
            Ok((output, if holds { 0 } else { 1 }))
