...
impl ScanFlightTicket {
    ...
    pub fn validate_flight_info(&self, info: &FlightInfo) -> Result<()> {
    ...
    pub fn schema_from_schema_result_for_flight_info(
        &self,
        schema_result: SchemaResult,
        info: &FlightInfo,
    ) -> Result<Schema> {
        let schema = schema_result.try_into().map_err(|err| {
            EhdbError::InvalidState(format!("decode scan SchemaResult IPC bytes: {err}"))
        })?;
        validate_scan_flight_info_for_schema(info, self, &schema)?;
        Ok(schema)
    }
...
impl ArrowScanResult {
    ...
    pub fn to_flight_data(&self) -> Result<Vec<FlightData>> {
        let mut stream = batches_to_flight_data(self.schema.as_ref(), self.batches.clone())
            .map_err(|err| {
                EhdbError::InvalidState(format!("encode scan result flight data: {err}"))
            })?;
        let first = stream.first_mut().ok_or_else(|| {
            EhdbError::InvalidState("encoded scan result produced no flight data".to_string())
        })?;
        first.app_metadata = SCAN_FLIGHT_RESULT_STREAM_VERSION.as_bytes().to_vec().into();
        Ok(stream)
    }
    ...
    pub fn from_flight_data_for_schema_info_and_ticket(
        flight_data: &[FlightData],
        expected_schema: &Schema,
        info: &FlightInfo,
        ticket: &ScanFlightTicket,
    ) -> Result<Self> {
        ticket.validate_flight_info_schema(info, expected_schema)?;
        Self::from_flight_data_for_info_and_ticket(flight_data, info, ticket)
    }

    pub fn from_flight_data_for_schema_result_info_and_ticket(
        flight_data: &[FlightData],
        schema_result: SchemaResult,
        info: &FlightInfo,
        ticket: &ScanFlightTicket,
    ) -> Result<(Schema, Self)> {
        let schema = ticket.schema_from_schema_result_for_flight_info(schema_result, info)?;
        let result =
            Self::from_flight_data_for_schema_info_and_ticket(flight_data, &schema, info, ticket)?;
        Ok((schema, result))
    }
    ...
    pub fn from_batches_for_schema_info_and_ticket(
        batches: Vec<RecordBatch>,
        expected_schema: &Schema,
        info: &FlightInfo,
        ticket: &ScanFlightTicket,
        ...
        ticket.validate_flight_info_schema(info, expected_schema)?;
        Self::from_batches_for_info_and_ticket(batches, info, ticket)
    }
...
fn validate_scan_flight_info_for_ticket(
    info: &FlightInfo,
    expected_ticket: &ScanFlightTicket,
) -> Result<()> {
    expected_ticket.validate_version()?;
    validate_scan_latest_table_request(&expected_ticket.request)?;
    validate_scan_flight_info(info)?;
...
impl RetrievalContextPayloadExecutionReceiptEventPayload {
    ...
    pub fn decode(bytes: &[u8]) -> Result<Self> {
        ...
        if canonical.as_slice() != bytes {
            return Err(EhdbError::InvalidState(
                "retrieval context execution receipt event payload must use canonical EHDB encoding"
                    .to_string(),
            ));
        }
        Ok(payload)
...
mod tests {
    ...
    #[test]
    fn retrieval_context_receipt_event_payload_rejects_noncanonical_json() {
        let receipt_payload = RetrievalContextPayloadExecutionReceiptPayload::new(
            RetrievalContextPayloadExecutionSummary {
                request_payload_bytes: 128,
                result_payload_bytes: 256,
        ...
        let event =
            RetrievalContextPayloadExecutionReceiptEventPayload::new(receipt_payload).unwrap();
        let pretty_event = serde_json::to_vec_pretty(&event).unwrap();

        assert_ne!(pretty_event, event.encode().unwrap());
        assert!(matches!(
            RetrievalContextPayloadExecutionReceiptEventPayload::decode(&pretty_event).unwrap_err(),
            EhdbError::InvalidState(_)
        ));
        assert_eq!(
            RetrievalContextPayloadExecutionReceiptEventPayload::decode(&event.encode().unwrap())
                .unwrap(),
            event
        );
    }
    ...
    #[test]
    fn scan_result_flight_data_rejects_missing_or_unsupported_stream_metadata() {
        let (log_path, object_root, runtime, store, tenant, namespace, table_name) =
            seeded_table("service-flight-data-metadata");
        let result = LocalArrowScanService::default()
            .scan_latest(
                &runtime,
                &store,
        ...
        let mut missing = result.to_flight_data().unwrap();
        missing[0].app_metadata = Vec::new().into();
        assert!(matches!(
            ArrowScanResult::from_flight_data(&missing).unwrap_err(),
            EhdbError::InvalidState(_)
        ));

        let mut unsupported = result.to_flight_data().unwrap();
    ...
    #[test]
    fn scan_flight_ticket_rejects_unexpected_flight_info() {
        let (log_path, object_root, runtime, store, tenant, namespace, table_name) =
            seeded_table("service-flight-info-expected-ticket");
        let request = ScanLatestTableRequest {
            tenant,
            namespace,
            table_name,
    ...
    fn scan_result_rejects_flight_data_info_mismatches() {
        ...
        let flight_data = result.to_flight_data().unwrap();
        let info = result.to_flight_info(&ticket).unwrap();
        let schema_result = schema_result_from_schema(result.schema.as_ref()).unwrap();
        ...
        let (decoded_schema, decoded) =
            ...
            .unwrap();
        assert_eq!(&decoded_schema, result.schema.as_ref());
        assert_eq!(decoded.row_count, result.row_count);
        let decoded_from_batches = ArrowScanResult::from_batches_for_schema_info_and_ticket(
            result.batches.clone(),
            result.schema.as_ref(),
            &info,
            &ticket,
        )
        .unwrap();
        assert_eq!(decoded_from_batches.row_count, result.row_count);

        let mut other_request = ticket.request.clone();
        other_request.table_name = TableName::new("other_executions").unwrap();
        let other_ticket = ScanFlightTicket::new(other_request);
        assert!(matches!(
            ArrowScanResult::from_flight_data_for_schema_info_and_ticket(
                &flight_data,
                result.schema.as_ref(),
                &info,
                &other_ticket
        ...
        assert!(matches!(
            ArrowScanResult::from_batches_for_schema_info_and_ticket(
                result.batches.clone(),
                result.schema.as_ref(),
                &wrong_schema,
                &ticket,
        ...
        assert!(matches!(
            ArrowScanResult::from_flight_data_for_schema_result_info_and_ticket(
                &flight_data,
                schema_result_from_schema(&Schema::new(vec![Field::new(
                    "other",
                    DataType::Utf8,
                    true,
                )]))
                .unwrap(),
                &result.to_flight_info(&ticket).unwrap(),
                &ticket,
            )
            .unwrap_err(),
            EhdbError::InvalidState(_)
    ...
    #[test]
    fn scan_result_flight_info_rejects_empty_schema_metadata() {
        let (log_path, object_root, runtime, store, tenant, namespace, table_name) =
            seeded_table("service-flight-info-schema-validation");
        let request = ScanLatestTableRequest {
            tenant,
            namespace,
            table_name,
    ...
    fn local_flight_service_returns_info_schema_and_do_get_stream() {
        ...
        assert_eq!(response_schema, schema);
        assert_eq!(schema.field(0).name(), "execution_id");
    ...
    async fn flight_client_reads_scan_over_loopback_listener() {
        ...
        let batches = client
            ...
            .await
            .unwrap();
        let decoded = ArrowScanResult::from_batches_for_schema_info_and_ticket(
            batches.clone(),
            &schema,
            &info,
            &ticket,
